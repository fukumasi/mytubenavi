...
exports.getFeaturedVideos = async (req, res) => {
  ...
  try {
    const featuredVideos = await FeaturedVideo.find().populate('user', 'username channelName');
    console.log('Found featured videos:', JSON.stringify(featuredVideos, null, 2));
...
exports.addFeaturedVideo = async (req, res) => {
  ...
  try {
    ...
    console.log('New featured video added:', JSON.stringify(newFeaturedVideo, null, 2));
