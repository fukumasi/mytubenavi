...
exports.getMediumGenres = async (req, res, next) => {
  ...
  try {
    ...
    console.log(
      `Querying database for medium genres of large genre ${largeGenreSlug}`,
    );
    const largeGenre = await Genre.findOne({
      slug: largeGenreSlug,
      level: "large",
    });
    ...
    if (!genres || genres.length === 0) {
      ...
      return res
        .status(404)
        .json({ message: "No medium genres found for this large genre" });
    }
...
exports.getSmallGenres = async (req, res, next) => {
  ...
  try {
    ...
    console.log(
      `Querying database for small genres of medium genre ${mediumGenreSlug}`,
    );
    const mediumGenre = await Genre.findOne({
      slug: mediumGenreSlug,
      level: "medium",
    });
    ...
    if (!genres || genres.length === 0) {
      ...
      return res
        .status(404)
        .json({ message: "No small genres found for this medium genre" });
    }
