  ...
  const generateReport = async () => {
    try {
      ...
      const reportResult = {
        ...
        bySlotType: {} as Record<string, { count: number, amount: number }>,
        dailyTrends: [] as Array<{ date: string, count: number, amount: number }>
      };
...
/* 未使用関数 - 将来的に必要になる場合はコメントを外してください
const handleRefund = async (paymentId: string, reason: string): Promise<void> => {
  try {
    setLoading(true);
    
    // 選択された決済情報を取得
    if (!selectedPayment) {
      throw new Error('決済情報が選択されていません');
    }
    ...
    // 成功メッセージの設定
    setSuccessMessage('返金処理が正常に完了しました');
    ...
    setError(`返金処理に失敗しました: ${err.message}`);
  } finally {
    setLoading(false);
  }
};
...
const handlePageChange = (page: number) => {
  ...
  const showPaymentDetails = (payment: PromotionPayment) => {
    // TODO: 詳細表示モーダルを実装する場合はここに追加
    console.log('決済詳細:', payment);
  };
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-lg">
            <form onSubmit={applyFilter}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">掲載枠タイプ</label>
                  <select
                    name="slotType"
             ...
             <table className="min-w-full border border-gray-200">
               <thead>
                 <tr className="bg-gray-50">
                   ...
                   <th className="p-3 text-left font-medium text-gray-500 tracking-wider">掲載枠</th>
               ...
               <tbody className="bg-white divide-y divide-gray-200">
                 {payments.map((payment) => (
                   <tr key={payment.id} className="hover:bg-gray-50">
                     ...
                     <td className="p-3">
                       <div className="text-sm font-medium">{payment.user?.username || '不明'}</div>
                     </td>
                     <td className="p-3">
         ...
         {loading ? (
           ...
           <div className="overflow-x-auto">
             <table className="min-w-full border border-gray-200">
               <thead>
                 <tr className="bg-gray-50">
                   <th className="p-3 text-left font-medium text-gray-500 tracking-wider">プラン名</th>
                   <th className="p-3 text-left font-medium text-gray-500 tracking-wider">タイプ</th>
                   <th className="p-3 text-right font-medium text-gray-500 tracking-wider">価格</th>
                   <th className="p-3 text-center font-medium text-gray-500 tracking-wider">最大動画数</th>
                   <th className="p-3 text-left font-medium text-gray-500 tracking-wider">説明</th>
                   <th className="p-3 text-center font-medium text-gray-500 tracking-wider">アクション</th>
                 </tr>
               </thead>
               <tbody className="bg-white divide-y divide-gray-200">
             ...
             <div className="mb-6">
               ...
               <div className="overflow-x-auto">
                 <table className="min-w-full border border-gray-200">
                   <thead>
                     <tr className="bg-gray-50">
                       <th className="p-3 text-left font-medium text-gray-500 tracking-wider">ステータス</th>
                       <th className="p-3 text-right font-medium text-gray-500 tracking-wider">件数</th>
                       <th className="p-3 text-right font-medium text-gray-500 tracking-wider">金額</th>
                       <th className="p-3 text-right font-medium text-gray-500 tracking-wider">割合</th>
                     </tr>
                   </thead>
                   <tbody className="bg-white divide-y divide-gray-200">
                     {Object.entries(reportData.byStatus).map(([status, data]: [string, any]) => (
             ...
             <div className="mb-6">
               ...
               <div className="overflow-x-auto">
                 <table className="min-w-full border border-gray-200">
                   ...
                   <tbody className="bg-white divide-y divide-gray-200">
                     {Object.entries(reportData.bySlotType).map(([type, data]: [string, any]) => (
             ...
             <div>
               ...
               <div className="overflow-x-auto">
                 <table className="min-w-full border border-gray-200">
                   <thead>
                     <tr className="bg-gray-50">
                       <th className="p-3 text-left font-medium text-gray-500 tracking-wider">日付</th>
                       <th className="p-3 text-right font-medium text-gray-500 tracking-wider">件数</th>
                       <th className="p-3 text-right font-medium text-gray-500 tracking-wider">金額</th>
                     </tr>
                   </thead>
                   <tbody className="bg-white divide-y divide-gray-200">
                     {reportData.dailyTrends.map((day: any) => (
