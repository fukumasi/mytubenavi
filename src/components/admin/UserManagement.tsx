...
const UserManagement: React.FC = () => {
  ...
  }, [page, perPage, searchParams, sortField, sortOrder]);
...
const updateUserRole = async (userId: string, role: string) => {
  try {
    ...
    setError(`ユーザー権限の更新に失敗しました: ${err.message || 'エラーが発生しました'}`);
  }
};
...
const updatePremiumStatus = async (userId: string, is_premium: boolean) => {
  try {
    ...
    setError(`プレミアムステータスの更新に失敗しました: ${err.message || 'エラーが発生しました'}`);
  }
};
  ...
  const updateAccountStatus = async (userId: string, active: boolean) => {
    try {
      ...
      setError(`アカウント状態の更新に失敗しました: ${err.message || 'エラーが発生しました'}`);
    }
  };
...
const handleBulkRoleUpdate = async () => {
  ...
  try {
    ...
    setError(`一括権限更新に失敗しました: ${err.message || 'エラーが発生しました'}`);
  } finally {
...
const handleBulkPremiumUpdate = async () => {
  ...
  try {
    ...
    setError(`一括プレミアムステータス更新に失敗しました: ${err.message || 'エラーが発生しました'}`);
  } finally {
    setProcessingBulk(false);
  }
};
  ...
  const handleBulkActiveUpdate = async () => {
    ...
    try {
      ...
      setError(`一括アカウント状態更新に失敗しました: ${err.message || 'エラーが発生しました'}`);
    } finally {
      setProcessingBulk(false);
    }
  };
