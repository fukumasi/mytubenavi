...
const VerificationGuard: React.FC<VerificationGuardProps> = ({
  ...
  const [userGender] = useState<string | null>(null);
  ...
  }, [requiredLevel, userGender, isPremium]);
      ...
      return (
        <div className="bg-white rounded-lg shadow-md p-8 max-w-md mx-auto text-center">
          ...
          <p className="text-gray-600 mb-6">
            {fallbackMessage || 'この機能を利用するには認証が必要です。プロフィールの設定と認証を完了してください。'}
          </p>
          <Link to="/profile/settings" className="block w-full bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 px-4 rounded-lg">
            プロフィール設定へ
          </Link>
        </div>
      );
    ...
    if (userGender === 'female') {
      return (
        <div className="bg-white rounded-lg shadow-md p-8 max-w-md mx-auto text-center">
          ...
          <p className="text-gray-600 mb-6">
            {femaleMessage || 'この機能を利用するには電話番号認証が必要です。認証完了後は無料でマッチング機能をお使いいただけます。'}
          </p>
          ...
          <Link to="/profile/verification" className="block w-full bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 px-4 rounded-lg">
            認証ページへ進む
          </Link>
        </div>
      );
      ...
      return (
        <div className="bg-white rounded-lg shadow-md p-8 max-w-md mx-auto text-center">
          ...
          <p className="text-gray-600 mb-6">
            {maleMessage || 'この機能を利用するにはプレミアム会員登録が必要です。月額980円でマッチング機能を制限なくご利用いただけます。'}
          </p>
          <div className="space-y-2">
          ...
          <Link to="/premium" className="block w-full bg-indigo-500 hover:bg-indigo-600 text-white font-medium py-2 px-4 rounded-lg">
            プレミアム会員登録へ
          </Link>
        </div>
      );
