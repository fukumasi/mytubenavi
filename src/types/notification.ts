...
export type NotificationType = 
 ...
 | 'connection_request'  // 接続リクエスト通知を追加
 | 'connection_accepted' // 接続承認通知を追加
 | 'connection_rejected' // 接続拒否通知を追加
 | 'match';     // マッチング成立通知を追加
...
export interface NotificationMetadata {
 ...
 connection_data?: {         // 接続関連のメタデータを追加
   ...
   request_date?: string;
   action_required?: boolean; // ユーザーアクションが必要か
 };
 interaction_history?: {
...
export interface NotificationPreference {
 ...
 matching_notifications: boolean; // マッチング通知
 message_notifications: boolean; // メッセージ通知
 connection_notifications: boolean; // 接続リクエスト通知を追加
...
export interface NotificationAction {
 id: string;
 label: string;
 type: 'accept' | 'reject' | 'view' | 'reply' | 'dismiss' | 'link';
 url?: string;
...
// 接続リクエスト通知の強化用コンポーネントプロパティ型
export interface ConnectionRequestNotificationProps {
