...
-- Create improved search function with correct parameters
...
BEGIN
  ...
  WITH filtered_videos AS (
    ...
    ORDER BY
      CASE 
        WHEN sort_by = 'date' THEN v.published_at
        WHEN sort_by = 'rating' THEN v.rating
        WHEN sort_by = 'views' THEN v.view_count
        ELSE ts_rank(
          to_tsvector('japanese', v.title || ' ' || v.description),
          to_tsquery('japanese', search_query)
        )
      END DESC
