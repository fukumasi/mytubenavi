...
-- Create improved search function with correct ORDER BY syntax
...
BEGIN
  ...
  WITH filtered_videos AS (
    ...
    ORDER BY
      CASE sort_by
        WHEN 'date' THEN v.published_at::text
        WHEN 'rating' THEN v.rating::text
        WHEN 'views' THEN v.view_count::text
        ELSE
          CASE
            WHEN v.title ILIKE '%' || search_query || '%' THEN '0' || v.published_at::text
            WHEN v.description ILIKE '%' || search_query || '%' THEN '1' || v.published_at::text
            ELSE '2' || v.published_at::text
          END
      END DESC NULLS LAST
