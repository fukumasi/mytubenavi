...
"CREATE POLICY admin_manage_profile_updates ON public.profile_updates FOR * TO 16479 USING ((EXISTS ( SELECT 1
   FROM profiles
...
"CREATE POLICY ""Users can view analytics for their bookings"" ON public.slot_booking_analytics FOR r TO 0 USING ((EXISTS ( SELECT 1
   FROM slot_bookings
  WHERE ((slot_bookings.id = slot_booking_analytics.booking_id) AND (slot_bookings.user_id = auth.uid())))));"
"CREATE POLICY ""Admins can manage all booking analytics"" ON public.slot_booking_analytics FOR * TO 0 USING ((EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::text)))));"
null
"CREATE POLICY ""Enable update for admin users"" ON public.promotion_slots FOR w TO 16479 USING ((EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::text)))));"
null
...
"CREATE POLICY ""Admins can update any profile"" ON public.profiles FOR w TO 16479 USING ((EXISTS ( SELECT 1
   FROM profiles profiles_1
  WHERE ((profiles_1.id = auth.uid()) AND (profiles_1.role = 'admin'::text)))));"
"CREATE POLICY ""Admins can select profile updates"" ON public.profile_updates FOR r TO 16479 USING ((EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'admin'::text)))));"
null
null
...
null
"CREATE POLICY ""Enable delete for youtuber users"" ON public.promotion_slots FOR d TO 16479 USING ((EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'youtuber'::text)))));"
"CREATE POLICY ""メッセージの送信者または受信者のみファイル"" ON public.message_attachments FOR r TO 0 USING ((EXISTS ( SELECT 1
   FROM (messages m
...
"ALTER TABLE public.youtuber_profiles ADD POLICY ""Users can create YouTuber profile once_with_check"" FOR a TO 0 WITH CHECK (((auth.uid() = id) AND (NOT (EXISTS ( SELECT 1
   FROM youtuber_profiles youtuber_profiles_1
  WHERE (youtuber_profiles_1.id = auth.uid()))))));"
...
"ALTER TABLE public.promotion_slots ADD POLICY ""Enable insert for youtuber users_with_check"" FOR a TO 16479 WITH CHECK ((EXISTS ( SELECT 1
   FROM profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'youtuber'::text)))));"
...
"ALTER TABLE public.slot_booking_analytics ADD POLICY ""Users can insert analytics for their bookings_with_check"" FOR a TO 16479 WITH CHECK (((EXISTS ( SELECT 1
   FROM slot_bookings
  WHERE ((slot_bookings.id = slot_booking_analytics.booking_id) AND (slot_bookings.user_id = auth.uid())))) OR (EXISTS ( SELECT 1
   FROM profiles
  WHERE (profiles.id = auth.uid())))));"
"ALTER TABLE public.message_attachments ADD POLICY ""メッセージの送信者のみファイル添付を作成可_with_check"" FOR a TO 0 WITH CHECK ((EXISTS ( SELECT 1
   FROM messages m
